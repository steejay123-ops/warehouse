...
> [!IMPORTANT]
> **قانون ۲ (صفر مهاجرت داده):**
> جابجایی مدل بین اپ‌ها فقط با `migrations.SeparateDatabaseAndState` و تثبیت `db_table`/`db_column` انجام می‌شود. هیچ `ALTER TABLE` سنگین، هیچ کپی جدول و هیچ بازسازی ایندکس روی دادهٔ تولیدی رخ نمی‌دهد.
