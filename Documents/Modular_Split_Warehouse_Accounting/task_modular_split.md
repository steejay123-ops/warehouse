...
- [ ] <!-- id: 0 --> **فاز ۰: نصب حصار پیش از هر تغییر رفتاری**
  - [ ] <!-- id: 1 --> افزودن `pyproject.toml` با اعلام سه توزیع `wh-platform`، `wh-warehouse` و `wh-accounting` (در توسعه همه کنار هم نصب)
  - [ ] <!-- id: 2 --> افزودن `.importlinter` با قراردادهای C1 (لایه‌بندی)، C2 (استقلال ماژول‌ها)، C3 (پاکی هسته) و C4 (استثنای مهارشده) در حالت report-only
  - [ ] <!-- id: 3 --> اجرای `lint-imports` و ثبت snapshot تعداد نقض‌های فعلی به‌عنوان مبنای مقایسه
  - [ ] <!-- id: 4 --> پیاده‌سازی ایجنت نگهبان ۲۹ (`scripts/e2e/guardian_modularization.py`) با تحمیل کاهش یکنوای نقض‌ها
  - [ ] <!-- id: 5 --> تایید عدم رگرسیون: هیچ تست موجودی سرخ نشده و نگهبان‌های ۱ تا ۲۸ سبزند
