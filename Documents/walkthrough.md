...
## ۵. بازرسی و طرح ارتقای ناوگان جدید (Vehicle New Fleet Refactor Inspection & Plan)
...
* **وضعیت:** ⏳ بازرسی عمیق تکمیل و مستندسازی شد. طبق دستور کاربر («فعلا کد ها را تغییر نده فقط طرح بنویس»)، هیچ کدی در پروژه تغییر نکرده و سیستم در انتظار تایید کاربر جهت شروع فاز پیاده‌سازی است.

| لایه معماری | تعداد موارد شناسایی‌شده | وضعیت فعلی | خروجی برنامه‌ریزی‌شده |
| :--- | :---: | :--- | :--- |
| **۱. لایه رابط کاربری (UI/UX)** | ۷ مورد | فرم درون‌خطی ۵۰۰px، فاقد هدر استاندارد، پلاک متنی نامعتبر | مودال شناور + Bottom Sheet، هدر شیشه‌ای چسبان، پلاک استاندارد ۴ قسمتی ایران |
| **۲. لایه گردش کار و کارتابل** | ۵ مورد | هاردکد draft، فقدان ارسال به سرپرست، قفل بودن ویرایش مصوب | دکمه دوگانه پیش‌نویس/ارسال، آیکون موشک، اتصال به `VehicleChangeRequest` |
| **۳. لایه امنیت و بک‌اند** | ۵ مورد | باگ تنزل رتبه خودرو در تایید مدیر، فقدان فیلتر قلمرو بخش | رفع باگ در `views.py`، فیلتر `UserSectionAssignment`، جلوگیری از حذف فیزیکی |
